    ...
    i.infinitescroll.prototype = {
        _binding: function g(F) {
            ...
            if (F === "unbind") {
                this.options.binder.unbind(
                    "smartscroll.infscr." + D.options.infid
                );
            } else {
                this.options.binder[F](
                    "smartscroll.infscr." + D.options.infid,
                    function () {
                        D.scroll();
                    }
        ...
        _create: function t(F, J) {
            ...
            G.loading.msg =
                G.loading.msg ||
                i(
                    '<div id="infscr-loading" class="jl_spinner"><i class="fa fa-circle-o-notch"></i><img alt="Loading..." src="' +
                        G.loading.img +
                        '" /><div>' +
                        G.loading.msgText +
                        "</div></div>"
            ...
            if (G.pixelsFromNavToBottom === k) {
                G.pixelsFromNavToBottom =
                    i(document).height() - i(G.navSelector).offset().top;
            }
            var E = this;
            G.loading.start =
                G.loading.start ||
                function () {
                    ...
                    G.loading.msg.appendTo(G.loading.selector).show(
                        G.loading.speed,
                        i.proxy(function () {
                            this.beginAjax(G);
                        }, E)
            ...
            G.callback = function (K, L) {
                if (!!G.behavior && K["_callback_" + G.behavior] !== k) {
                    K["_callback_" + G.behavior].call(
                        i(G.contentSelector)[0],
                        L
                    );
                }
                if (J) {
        ...
        _loadcallback: function c(H, I) {
            ...
            if (G.animate) {
                var E =
                    i(o).scrollTop() +
                    i("#infscr-loading").height() +
                    G.extraScrollPx +
                    "px";
                i("html,body").animate({ scrollTop: E }, 800, function () {
        ...
        beginAjax: function B(G) {
            ...
            D =
                typeof I === "function"
                    ? I(G.state.currPage)
                    : I.join(G.state.currPage);
            E._debug("heading into ajax", D);
            K =
                G.dataType === "html" || G.dataType === "json"
                    ? G.dataType
                    : "html+callback";
            ...
            switch (K) {
                ...
                case "html":
                    E._debug(
                        "Using " + K.toUpperCase() + " via $.ajax() method"
                    );
                    i.ajax({
                        url: D,
                        dataType: G.dataType,
                        complete: function H(L, M) {
                            J =
                                typeof L.isResolved !== "undefined"
                                    ? L.isResolved()
                                    : M === "success" || M === "notmodified";
                            if (J) {
                ...
                case "json":
                    ...
                    i.ajax({
                        ...
                        success: function (N, O, M) {
                            J =
                                typeof M.isResolved !== "undefined"
                                    ? M.isResolved()
                                    : O === "success" || O === "notmodified";
                            if (G.appendCallback) {
                                if (G.template !== k) {
                                    var L = G.template(N);
                                    F.append(L);
